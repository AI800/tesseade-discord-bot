...
async def create_duel_channel(ctx, channel_data: dict):
    ...
    # Create the channel
    category = discord.utils.get(guild.categories, name="Duels")  # Optional: create under a category
    ...
    try:
        ...
        await ctx.send(f"⚔️ Duel channel created: {channel.mention}\nPlease move there to begin!")
        
        return channel
        
    except Exception as e:
        print(f"Error creating duel channel: {e}")
        await ctx.send(f"❌ Failed to create duel channel: {e}")
        return None
...
async def schedule_channel_deletion(channel_id: int, delay: int = 600):
    ...
    if duel_channels[channel_id]['delete_task']:
...
async def check_duel_turn(ctx, duel_id: int, user_id: str) -> bool:
    """Check if it's the user's turn in the duel"""
    # This would query the database to check current_turn_player
    # Return True if it's their turn, False otherwise
    pass

# Update your command handler to include duel context
@bot.command(name='duel')
async def duel_command(ctx, *args):
    """Handle duel commands with channel context"""
    
    # Check if in a duel channel
    duel_id = None
    if ctx.channel.id in duel_channels:
        duel_id = duel_channels[ctx.channel.id]['duel_id']
        
        # For attack/use commands, verify it's the player's turn
        if args and args[0] in ['attack', 'use']:
            # Add turn verification here
            pass
    
    # Prepare data for PHP
    data = {
        'command': f'!duel {" ".join(args)}',
                    ...
                    if isinstance(result, dict):
                        if result.get('success'):
                            ...
                            if result.get('create_duel_channel'):
                                channel = await create_duel_channel(ctx, result['channel_data'])
                                if not channel:
                                    return
                            ...
                            await ctx.send(result.get('message', '❌ Command failed'))
                    else:
...
# Background task to clean up expired duels
...
# Add the cleanup task to bot startup
@bot.event
async def on_ready():
